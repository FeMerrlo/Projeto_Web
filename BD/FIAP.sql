...
create table produto 
(cod_prod number(4) constraint pro_cod_prod_pk primary key, 
unidade varchar2(3),descricao varchar2(20),val_unit number(10,2));
...
5.4) Quais pedidos foram feitos pelos cliente de código maior que 500? Mostre o
     ...
     de forma crescente
5.5) crie um relatório que exiba o código do cliente e seu nome
,desde que estejam no intervalo do código com inicio em 180 atpr 720 (inclusive) 


5.6) crie um relatório que exiba o código do cliente e seu nome,
desde que estejam no intervalo do código com inicio em 180 atpr 720


5.7) crie um relatório que exiba o nome do cliente,
desde que estejam no intervalo de B até F (inclusive)


5.8) quem são os vendedores com salário superior a R$2.000,00 que pertençam a comissão
A ou C e que seu código esteja no intervalo de 500 a 800

Aula 9 - 19/04 gabarito exercicios


5.2) select nome_cli, uf from cliente where upper(uf) = 'SP';
...
5.5) select cod_clie, nome_clie from cliente where cod_clie >180 and cod_clie <720 order by 1;

5.6) select cod_clie, nome_clie from cliente where cod_clie between 180 and 720 order by 1;

5.7) select cod_clie, nome_clie from cliente where nome_clie between 'B' and 'F' order by 1;

5.8) select nome_ven, salario_fixo, comissao from vendedor where salario_fixo > 2000 and comissao != 'B' order by 1;